						...
						<form action="{{ route('torneos.update', ['slug' => $tournament->slug]) }}" method="POST" class="form" id="formTournament">
							...
							<div class="row">
								<div class="form-group col-lg-8 col-md-8 col-12">
									...
									<input class="form-control" type="text" name="title" required placeholder="Introduzca un nombre" value="{{ $tournament->title }}">
								</div>
								...
								<div class="form-group col-lg-4 col-md-4 col-12">
									...
									<select class="form-control" name="year" required>
										...
										@for ($i = 2020; $i < 2051; $i++)
											<option value="{{ $i }}" @if($i==$tournament->year) selected @endif>{{ $i }}</option>
										@endfor
									</select>
								</div>
								...
								<div class="form-group col-lg-6 col-md-6 col-12">
									...
									<select class="form-control" name="days" required>
										<option value="">Seleccione</option>
										@for ($i = 1; $i < 51; $i++)
											<option value="{{ $i }}" @if($i==$tournament->day) selected @endif>{{ $i }}</option>
										@endfor
									</select>
								</div>
								...
								<div class="form-group col-12">
									<div class="btn-group" role="group">
										<button type="submit" class="btn btn-primary" action="tournament">Actualizar</button>
